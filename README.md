...
│       ├── index.ts             # エントリポイント
│       ├── types.ts             # 型定義
│       ├── middleware/auth.ts   # セッション認証・パスワードハッシュ処理
│       └── routes/
...
- メール/パスワード認証（D1裏付けのセッション管理、有効期限30日、ログアウトで即時失効）
...
- **パスワードハッシュ**: PBKDF2（SHA-256、100,000回反復）+ ユーザーごとのランダムソルトで保存。比較はタイミング攻撃を避けるため定数時間比較を使用しています。
  - ⚠️ 過去のバージョンでは固定ソルト+SHA-256という脆弱な方式を使用していました。この方式変更により、**過去に作成したアカウントのパスワードは検証できなくなります**。本番運用前であれば、`worker/migrations/002_clear_test_data_password_migration.sql`をD1のConsoleタブで実行し、テストユーザーを一度クリアしてから運用を開始してください。
- **パスワード強度チェック**: 登録・変更時にサーバー側で検証します。8文字以上・英字と数字の組み合わせ必須・よくある単純パスワードの拒否・同一文字の連続パターン拒否・128文字以内。フロントエンド側にも即時フィードバック用の同等チェックを実装しています。
- **レート制限（ログイン・登録）**: D1の`rate_limits`テーブルで管理します。
  - ログイン: IPアドレス単位で15分間に20回まで、メールアドレス単位で15分間に8回まで
  - 登録: IPアドレス単位で1時間に10回まで
  - パスワード変更: ユーザーID単位で15分間に8回まで
  - ログイン成功時はメールアドレス単位のカウンタをリセット。ブロック中は正しいパスワードでもリクエストを拒否します。
- **入力バリデーション**: 各エンドポイントでフィールドの型・長さ・数値範囲・enumホワイトリスト検証を実施。`limit`/`offset`クエリパラメータには上限（200件）を設けています。
- **XSS対策**: ユーザー入力をHTMLに埋め込む箇所はすべて`esc()`でエスケープ。`onclick`属性への直接オブジェクト埋め込みは`data-*`属性＋`readDataAttr()`経由に変更。CSVインジェクション対策も実施済み。
- **セキュリティヘッダー**: `X-Frame-Options: DENY`、`X-Content-Type-Options`、`Referrer-Policy`、`Permissions-Policy`（カメラ・マイク・位置情報・支払い機能を無効化）、`Content-Security-Policy`（`connect-src`でAPIエンドポイント以外へのデータ送信をブロック）を設定しています。
- **セッション管理（即時失効）**: D1の`sessions`テーブルと紐づくセッション方式。ログアウト時にセッション行を即時削除、パスワード変更時は全セッション一括失効＋新トークン再発行を行います。セッション有効期限は30日。
- **権限管理**: ロール変更APIは`editor`/`viewer`のみ許可（`owner`への昇格不可、オーナー自身のロール変更も禁止）。
- **環境変数・Secret管理**: `JWT_SECRET`等の機密情報は`wrangler.toml`の`[vars]`に一切記述していません。Cloudflareダッシュボードの Settings → Variables でEncryptをONにして設定してください。
- **CORS**: `FRONTEND_URL`環境変数で指定したオリジンのみ許可。`CF-Connecting-IP`（偽装不可能なCloudflare付与のIPヘッダー）のみをレート制限に使用し、`X-Forwarded-For`（クライアントが偽装可能）は信頼しません。
...
- メールアドレスの実在確認（メール認証）は未実装。登録時に実在しないメールアドレスでもアカウントが作成できます。
- OCR機能（レシート読み取り）は設計書にありましたが、今回はスコープ外としています。Workers AIを使えば後から追加可能です。
- 外部公開APIトークン（`/api/v1/public/*`）のレート制限は未実装です（`api_tokens`テーブルに`rate_limit`カラムのみ用意）。
- レート制限の実装はD1ベースの固定ウィンドウ方式のため、極めて高頻度の並行リクエストでは数件程度の揺れが発生する可能性があります。
