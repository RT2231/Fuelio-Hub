...
- **セッション管理（ログアウトの即時失効）**: 認証トークンはJWTではなく、D1の`sessions`テーブルと紐づくセッション方式に変更しています。トークン自体は「ランダムなセッションID + HMAC署名」のみを持ち、実際にそのセッションが有効かどうかは毎リクエストD1に問い合わせて判定します。
  - **ログアウト**(`POST /auth/logout`)時に該当セッション行をD1から削除するため、トークンはその場で即座に使えなくなります（従来のJWT単体方式では、ログアウトしてもトークン自体は有効期限まで使えてしまう問題がありました）。
  - **パスワード変更時**は、そのユーザーの全セッション（他の端末でログイン中のものも含む）を一括で失効させ、変更操作を行ったクライアントにだけ新しいトークンを発行し直します（アカウント乗っ取りの被害を受けた場合の被害抑止）。
  - セッションの有効期限は30日です。期限切れのセッションは、次回そのトークンでアクセスがあった際にD1から自動的に削除されます。
...
- **ログイン試行のレート制限がありません**。同一メールアドレスへの連続したパスワード総当たり攻撃を防ぐ仕組みは未実装です。
- **メールアドレスの実在確認（メール認証）がありません**。登録時に実在しないメールアドレスでもアカウントが作成できます。
- **パスワード強度チェックが最小限です**（8文字以上のみ）。`12345678`のような単純なパスワードも許可されます。
